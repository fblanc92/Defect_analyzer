...
def visualize_boxes_and_labels_on_image_array(
  ...
  boxes_data=[] # list to be returned to be saved in csv file
  detected_classes=[]
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        display_str = ''
        if not skip_labels:
          if not agnostic_mode: # this is the case
  ...
  for box, color in box_to_color_map.items():
    ...
    res_w = image.shape[1]
    res_h = image.shape[0]
    # 'Date','Hour','Type','ymin','xmin','ymax','xmax','Area (relative px)','
    ...
    boxes_data.append([date,                                # date
                       time,                                # Time
                       detected_classes[j],                 # Type
    ...
    boxes_data = sorted(boxes_data,key=lambda x: (
      x[0][2],x[0][1],x[0][0],                              # sort by date
      x[1][0],x[1][1],x[1][2],                              # sort by time
      x[3][1]))                                             # sort by xmin pos
    
    j+=1
#    boxes_data.append([date, time,detected_classes[j],box[0],box[1],box[2],box[3],abs((box[0]-box[2])*(box[1]*box[3])),abs((box[0]-box[2])*(box[1]*box[3])) * res_w * res_h,abs((box[0]-box[2])*(box[1]*box[3])) * res_w * res_h / const_mm,  # Area (mm2)
#                       scores[j]*100
#                       ])      
#    
